...
type
  ...
  public
    ...
    function GetNextBuffer(aBuffer: PByte): PByte; override; overload;
    function GetNextBuffer(aBuffer: PByte; IncludeDeleted: boolean): PByte; overload;
...
begin
  ...
  csRowBuffer:
    begin
      {$ifdef PrintBuf}
      writeln('Delete RecNo ',GetRecNo(aBufID));
      {$endif}
...
begin
  GotoLast;
  {$ifdef PrintBuf}
  writeln('Append After Record = ',GetRecNo(aBufID));
  {$endif}
...
{      if (field.FieldNo < 0) or (field.FieldNo >= Length(FFieldNo2ColumnMap)) then
      ...
      fdRefreshOnInsert := (pfRefreshOnInsert in field.ProviderFlags) or
                           (TIBFieldDef(field.FieldDef).IdentityColumn) or
                            field.FieldDef.InternalCalcField;
      fdRefreshOnUpdate := (pfRefreshOnUpdate in field.ProviderFlags) or
                            field.FieldDef.InternalCalcField;
    end;
...
var buff: PByte;
begin
  writeln('Print Buffer List for ',Name);
  ...
  while buff <> nil do
  begin
    writeln('Rec No = ',GetRecNo(buff),' status = ',GetRecordStatus(buff));
    buff := GetNextBuffer(buff,true);
  end;
end;
{$endif}
